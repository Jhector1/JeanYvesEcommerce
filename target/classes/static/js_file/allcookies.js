...
class CookieManager {
    ...
    static async deleteUserItem(cookie) {
        try {
            return await fetch(`/${cookie.type}/artworks/delete-artwork/${cookie.id}`, {
                method: 'DELETE'
            });
        } catch (error) {
    ...
    static async addCookie(cookie, quantity) {
        try {
           // console.log(quantity);
            ...
            const response = await new JsonRequest().post(`/${cookie.type}/artworks/save`, {
                productId: cookie.id,
                quantity: quantity,
    ...
    static async removeCookie(cookie) {
        ...
        try {
            ...
            const response = await fetch(`/${cookie.type}/artworks/${userId}/${artworkId}`, {
                method: 'DELETE'
            });
...
class CookieClient {
    ...
    displayNumberLike(numAdd, type) {
        fetch(`/${type}/artworks/${checkCookie("user12345")}`).then(response => {
            if (!response.ok) {
    ...
    async displayArtworkDataFromDB(parent_element, list, common_op, type) {
        try {
            const response = await fetch(`/${type}/artworks/${checkCookie("user12345")}`);
...
class ElementManager {
    ...
    async displayElementStatusFromDB(imageUrl, operation, cookie) {
        try {
            const response = await fetch(`/${cookie.type}/artworks/${checkCookie("user12345")}/${cookie.id}`);
    ...
    async displayElementStatusOrPurchase(imageUrl, cookie, operation, element4 = null) {
        try {
            const response = await fetch("/api/artworks/purchase/" + cookie.id);
